...
function edit_value($product, $key)
{
    $value = is_array($product) ? ($product[$key] ?? '') : ($product->$key ?? '');
...
<head>
    ...
    <title>Edit Product | Inventory Desk</title>
    ...
    <style>
        :root { --ink: #262626; --paper: #f5f5f5; --coral: #4a4a4a; --line: #dedede; }
        ...
        .topbar-label { color: #718096; font: .66rem "DM Mono", monospace; letter-spacing: .12em; text-transform: uppercase; }
...
<main class="container py-4">
    <div class="row justify-content-center"><div class="col-lg-8">
        <div class="product-form-header"><div><h1 class="h3 mb-0">Edit product</h1></div><a href="<?= site_url('products'); ?>" class="btn btn-outline-secondary">&#8592; Back</a></div>
        ...
        <div class="card product-form-card border-0"><div class="card-body p-4">
            <form method="post" action="<?= site_url('products/update/' . (int) $product_id); ?>">
                <div class="mb-3"><label class="form-label" for="product_name"><span>01</span> Name</label><input class="form-control" id="product_name" name="product_name" maxlength="100" required value="<?= edit_value($product, 'product_name'); ?>"></div>
                <div class="mb-3"><label class="form-label" for="description"><span>02</span> Description</label><textarea class="form-control" id="description" name="description" rows="4"><?= edit_value($product, 'description'); ?></textarea></div>
                <div class="row"><div class="col-md-6 mb-3"><label class="form-label" for="price"><span>03</span> Price</label><input class="form-control" id="price" name="price" type="number" min="0" step="0.01" required value="<?= edit_value($product, 'price'); ?>"></div><div class="col-md-6 mb-3"><label class="form-label" for="quantity"><span>04</span> Quantity</label><input class="form-control" id="quantity" name="quantity" type="number" min="0" step="1" required value="<?= edit_value($product, 'quantity'); ?>"></div></div>
